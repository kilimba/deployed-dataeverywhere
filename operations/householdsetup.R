...
contactData <- consentData[,c("YEAR","HIVRefused","HIVContactedRate")]
contactData$HIVContactedRate <- ifelse(contactData$HIVRefused =='Refused',100-contactData$HIVContactedRate,contactData$HIVContactedRate)
names(contactData) <- c("YEAR","Contacted","HIVContactRate")
contactData$Contacted <- ifelse(contactData$Contacted == 'Consented',"Contacted","Not Contacted")


BSRefusalData <<- sqlQuery(conn,"--Eligible vs Refused Boundedstructures
WITH
el AS    --Eligible
                           (
                           SELECT
                           DATEPART(YYYY, se.AssignedDate) AS [YEAR] 
                           --, vq.DSRound
                           , 'Eligible' AS BSGroup
                           , COUNT(1) QtyBS
                           FROM dbo.vacQAllocations vq
                           JOIN dbo.DSRounds d ON d.ExtID = vq.DSRound
                           JOIN dbo.Events se ON se.IntID = d.StartEvent
                           WHERE vq.Acronym LIKE 'BSU%'
                           AND vq.Acronym NOT LIKE 'BSU%QC%'
                           GROUP BY DATEPART(YYYY, se.AssignedDate)--, vq.DSRound
                           ),
                           ...
                           , COUNT(1) QtyBS
                           FROM dbo.Refusals r
                           JOIN dbo.vacQAllocations vq ON vq.QuestionnaireAllocation = r.QuestionnaireAllocation
                           JOIN dbo.DSRounds d ON d.ExtID = vq.DSRound
                           JOIN dbo.Events se ON se.IntID = d.StartEvent
                           WHERE r.AvoidBSHowLong = 'AVI'
                           AND vq.RoundType = 1
                           GROUP BY DATEPART(YYYY, se.AssignedDate)--, vq.DSRound
                           )
                           ...
                           *
                           FROM el
                           LEFT JOIN rf on rf.[Year] = el.[Year] 
                           ORDER BY el.[Year] ASC;
                           ")

BSRefusalData$BSGroup.1 <- NULL
BSRefusalData[is.na(BSRefusalData)] <- 0
BSRefusalData$RefusalRate <- (BSRefusalData$QtyBS.1/BSRefusalData$QtyBS)*10000
