...
source("householdsetup.R")
source("individualsetup.R")
source("vasetup.R")
...
ui <- dashboardPage(
  dashboardHeader(title = "Data Everywhere: Operations Portal",
                  titleWidth = 450),
  dashboardSidebar(
    sidebarMenu(
  ...
  dashboardBody(
    tabItems(
      ...
      ,tabItem(tabName = "typeA",
               fluidRow(
                 infoBox("About", textOutput("captionhh"), icon = icon("info-circle"),width = 12)
               ),
               fluidRow(
      ...
      tabItem(tabName = "typeB",
              fluidRow(
                infoBox("About", textOutput("captionind"), icon = icon("info-circle"),width = 12)
              ),
              fluidRow(
      ...
      tabItem(tabName = "typeC",
              fluidRow(
                infoBox("About", textOutput("captionva"), icon = icon("info-circle"),width = 12)
              ),
              fluidRow(
    ...
    # If selected "verbal autopsy" screen:
      ...
      observe({
        if(!is.null(input$yearsva)){
          ...
          output$captionva <- renderText({
            return(paste("You are currently viewing progress gauges for VERBAL AUTOPSY level data operations activities for the year",year))
          })
